...
class ResUsers(models.Model):
    ...
    def _natura_print_get_default_template(self, model_name):
        ...
        if not pref or not pref.template_id:
            return False
        if pref.template_id.company_id and pref.template_id.company_id not in self.env.companies:
            return False
        if pref.template_id.is_product_specific:
            return False
...
class NaturaPrintUserTemplatePref(models.Model):
    ...
    template_id = fields.Many2one(
        "zpl.label.template",
        string="Template",
        domain="[('model_id', '=', model_id), ('is_product_specific', '=', False), '|', ('company_id', '=', False), ('company_id', 'in', allowed_company_ids)]",
    )
