...
class NaturaPrintLotLabelWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "template_id" in fields_list and not res.get("template_id"):
            template = False
            if lot_ids:
                lots = self.env["stock.lot"].browse(lot_ids).exists()
                template = self._get_product_override_template(lots.mapped("product_id.product_tmpl_id"))
            if not template:
    ...
    def _get_product_tmpl_ids(self):
        self.ensure_one()
        return self.line_ids.mapped("lot_id.product_id.product_tmpl_id").ids
    ...
    def _get_product_override_template(self, product_templates):
        ...
        if len(product_templates) != 1:
            return False
        template = product_templates._natura_print_get_default_template_for_model("stock.lot")
        if not template:
            return False
        return template

    @api.depends("line_ids", "line_ids.lot_id", "line_ids.lot_id.product_id")
    def _compute_available_template_ids(self):
        for wizard in self:
            wizard.available_template_ids = wizard._get_available_templates()

    @api.onchange("line_ids", "line_ids.lot_id")
    def _onchange_line_ids_apply_template(self):
        for wizard in self:
            ...
            preferred = wizard._get_product_override_template(
                wizard.line_ids.mapped("lot_id.product_id.product_tmpl_id")
            )
