...
class NaturaPrintMrpLabelWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "template_id" in fields_list and not res.get("template_id"):
            template = False
            if mo_ids:
                productions = self.env["mrp.production"].browse(mo_ids).exists()
                template = self._get_product_override_template(productions.mapped("product_id.product_tmpl_id"))
            if not template:
                template = self.env.user._natura_print_get_default_template("mrp.production")
            res["template_id"] = template.id if template else False
        return res

    def _get_product_tmpl_ids(self):
        self.ensure_one()
        return self.line_ids.mapped("production_id.product_id.product_tmpl_id").ids
    ...
    def _get_product_override_template(self, product_templates):
        ...
        if len(product_templates) != 1:
            return False
        template = product_templates._natura_print_get_default_template_for_model("mrp.production")
        if not template:
            return False
        return template

    @api.depends("line_ids", "line_ids.production_id", "line_ids.production_id.product_id")
    def _compute_available_template_ids(self):
        for wizard in self:
            wizard.available_template_ids = wizard._get_available_templates()

    @api.onchange("line_ids", "line_ids.production_id")
    def _onchange_line_ids_apply_template(self):
        for wizard in self:
            ...
            preferred = wizard._get_product_override_template(
                wizard.line_ids.mapped("production_id.product_id.product_tmpl_id")
            )
